...
## HH.ru: подключение готовым токеном

Если у работодателя уже есть рабочая авторизация hh.ru в другом приложении
(например, в Call AI) и её нельзя ломать повторным OAuth, владелец организации
вставляет `access_token` на странице «Интеграции → HH.ru → У меня уже есть
токен» (или `POST /api/integrations/hh/connect-token`). Сервис проверяет токен
запросом `GET /me`, сохраняет его шифрованным и **никогда не обновляет**: у такого
подключения нет `refresh_token`, а обновление пары инвалидировало бы токен у
приложения-источника. Срок по умолчанию — 14 дней с момента вставки (hh выдаёт
токены на 14 дней); когда он истечёт, статус подключения покажет ошибку, и токен
надо вставить заново. Синхронизация и диалог с откликнувшимися идут только по
вакансиям, привязанным в LeonIT (`HhVacancyLink`), поэтому вакансии, которые ведёт
другое приложение, сервис не трогает.

## ИИ-аватар (HeyGen)

Аватар платный (движок `avatar_iii` — около $1 за минуту готового видео),
поэтому включается ключом и флагами на сервере (таблица выше). Когда провайдер
настроен, новые вакансии создаются с включённым аватаром — интервьюер с лицом и
голосом вместо озвучки без видео; выключить его можно переключателем
«ИИ-аватар интервьюера» в настройках конкретной вакансии. Вакансии, созданные
до настройки провайдера, остаются без аватара, пока переключатель не включат.
