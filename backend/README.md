...
Финальный блок интервью: после основных вопросов система может задать до
`followups_max` уточнений по ответам, где рекрутер поставил `allows_followup`.
Вопросы придумывает модель роли `interviewer` по уже распознанным ответам —
без транскрипта уточнять нечего (`interviews/followups.py`).

Блок собирается один раз и дописывается в конец `question_snapshot` обычными
вопросами с полем `followup_of`, поэтому дальше работает штатный путь комнаты:
показ, запись, докачка, «дальше». Ответ на уточнение получает
`parent_answer_id` зачётного ответа на исходный вопрос, перезаписей нет.

Транскрипты приходят из очереди, поэтому у комнаты есть бюджет ожидания:
`GET /public/invitations/{token}/followups` отдаёт `{enabled, ready, pending,
wait_seconds}`, комната опрашивает его перед финалом не дольше
`FOLLOWUP_WAIT_S` (90 с) и показывает «Готовим уточняющие вопросы…». Не успели
или модель вернула пустой план — интервью завершается без уточнений; отметка
`settings_snapshot.followups_done` не даёт собрать блок дважды.

### `integrity/` — достоверность записи

Отвечает на вопрос «на что посмотреть внимательнее», а не «жульничал ли
кандидат». Никакого распознавания личности, анализа взгляда и «детекции
ИИ-текста»: только то, что система знает точно — события комнаты, серверные
тайминги и метаданные файла из ffprobe.

Наблюдения не хранятся: `rules.analyze` считает их при каждом чтении отчёта,
поэтому изменение правил сразу видно и на старых интервью. В базе живёт только
вердикт человека (`integrity_reviews`: интервью + код наблюдения + номер
вопроса → «подтверждено» или «ложное срабатывание» с комментарием).

| Код | Уровень | Когда |
|---|---|---|
| `away_during_answer` | обратить внимание | во время записи ответа окно теряло фокус: одна отлучка от 15 с или две от 4 с |
| `paste_during_answer` | обратить внимание | вставка от 40 символов во время ответа |
| `virtual_camera` | высокий риск | среди устройств есть виртуальная камера (OBS, ManyCam, Snap и т. п.) |
| `foreign_recorder` | высокий риск | в тегах контейнера подпись стороннего ПО и нет подписи браузера |
| `duration_mismatch` | обратить внимание | длительность файла и замер браузера расходятся на четверть и от 5 с |
| `multiple_faces` | обратить внимание | на двух и более снимках в кадре больше одного лица |
| `no_face` | обратить внимание | лицо не распознано более чем на 60 % снимков |
| `heartbeat_gap` | факт | пауза в сигналах комнаты от 20 с (слабая сеть, свёрнутая вкладка) |

Против ложных срабатываний: пороги по длительности и повторам, окно записи
ответа (что было между вопросами — не в счёт), «факт» вместо «наблюдения» там,
где объяснение обычно техническое, и отсутствие правила, если сигналов не было
вовсе (например, телеметрия выключена — молчим, а не пугаем).

Число лиц считает браузер кандидата (`FaceDetector`, только Chromium): кадры
никуда не уходят, на сервер отправляется одно число. Нет детектора — нет
наблюдения.

Ручки: `GET /interviews/{id}/integrity` (`report.read`) и
`POST /interviews/{id}/integrity/review` (`report.decide`, тело — код
наблюдения, номер вопроса, вердикт, комментарий). Отмеченные ложными
наблюдения не влияют на итоговый уровень и не попадают в `flags_rate` на
дашборде. В отчёте по ссылке детали видны, только если владелец ссылки включил
`include_integrity`.

### `notifications/` — письма

Все письма проходят через outbox (`email_messages`) и задачу `email.send`:
запись в базе появляется в одной транзакции с событием, отправка — отдельно и с
повторами, а вкладка «Письма» в кабинете показывает и текст, и статус. Без SMTP
(`EMAIL_MODE=console`) письма просто остаются в outbox — ссылку из приглашения
можно скопировать оттуда.

| Письмо | `kind` | Когда |
|---|---|---|
| Приглашение | `interview.invitation` | приглашение кандидата и «отправить ссылку заново» |
| Напоминание | `interview.reminder` | тик воркера: ссылка истекает в ближайшие 48 часов, кандидат не закончил; один раз на интервью (`interviews.reminder_sent_at`) |
| Интервью завершено | `interview.completed` | кандидат дошёл до конца — рекрутеру |
| Заключение готово | `interview.evaluation_ready` | оценка сохранена — пригласившему рекрутеру и нанимающим менеджерам, у кого вакансия в периметре; повторная оценка шлёт «обновлено» |
| Обратная связь кандидату | `interview.candidate_feedback` | по настройке вакансии: `auto_after_days` — через N дней после заключения, `after_decision` — через N дней после решения, `off` — не шлём |

Напоминание выпускает новую ссылку (токен хранится хешем, старый перестаёт
работать) — как кнопка «Отправить ссылку заново». Обратная связь ставится
задачей с `run_after` и ключом `feedback:<interview>`; перед отправкой заново
проверяются настройка вакансии, наличие решения (для `after_decision`), текст
заключения и отписка — задача, поставленная неделю назад, не отправит письмо по
устаревшим условиям.

**Отписка.** В письме с обратной связью есть ссылка `PUBLIC_URL/unsubscribe/<токен>`:
токен подписан на `JWT_SECRET` (тип `unsubscribe`, срок год), отдельной таблицы
не нужно. Ручка `POST /api/public/unsubscribe/{token}` ставит
`candidates.unsubscribed_at`, снимает согласие на рассылку вакансий и пишет
отказ в журнал согласий; повторный вызов идемпотентен, подделка — 404. Страница
`/unsubscribe/<токен>` просит подтверждение кнопкой: почтовые клиенты
«прокликивают» ссылки заранее, и без явного шага человек отписался бы
случайно. Служебные письма (приглашение, напоминание, «интервью завершено»,
«заключение готово») отписке не подлежат — без них процесс найма не работает.
