...
def build_messages(text: str, company_context: str) -> list[Message]:
    parts = [
        ...
        "из 4–6 компетенций: родственные технологии объединяй в одну компетенцию, вес "
        "1–5 (5 — ядро роли, 2 — коммуникация), четыре якорных уровня, каждый описывает, "
        "что кандидат говорит в устном ответе («называет…», «приводит пример…», "
        "«объясняет компромисс…»), без «не имеет опыта»; id — латиницей в snake_case. "
        "Составь 5–7 вопросов для устного ответа на камеру 2–3 минуты без кода: каждый "
        "об одной теме, максимум два предложения, без перечисления подтем; чередуй "
        "случай из опыта, рассуждение и объяснение устройства; каждая компетенция "
        "покрыта хотя бы одним вопросом; у вопроса 3–4 конкретных expected_points и "
        "competency_ids из рубрики. В notes перечисли, чего в тексте не хватило "
        "(зарплата, формат работы, грейд) и что рекрутёру стоит проверить. Отвечай "
        "по-русски.",
