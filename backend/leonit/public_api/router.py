...
@router.get(
    ...
    description=(
        "Кандидаты, завершившие интервью, в порядке соответствия: сначала те, кому модель "
        "поставила `needs_check` (их нужно посмотреть человеку), затем по убыванию "
        "`fit_score`. Решение человека (`decision`) на порядок не влияет.\n\n"
